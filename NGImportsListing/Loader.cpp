...
bool loader::parse(E32Image& image)
{
	...
	{
		std::cout << "Non-fixed address executables aren't supported." << std::endl;
		return false;
	}
	...
	{
		std::cout << "Only basic header type is supported." << std::endl;
		return false;
	}
	...
	{
		std::cout << "EKA2 entry point type is unsupported." << std::endl;
		return false;
	}
	...
	{
		std::cout << "Other import formats not implemented." << std::endl;
		return false;
	}
	...
	if (image.abi != 0)
	{
		std::cout << "EABI ABI is unsupported." << std::endl;
		return false;
	}
	...
	{
		std::cout << "Data relocation not supported" << std::endl;
		return false;
	}
	...
	{
		std::cout << "Invalid E32Image signature." << std::endl;
		return false;
	}
	...
	{
		std::cout << "UID checksum mismatch." << std::endl;
		return false;
	}
	...
	{
		std::cout << "DLL count is 0, please report this. " << image.import_count << std::endl;
		return false;
	}*/
	...
	{
		std::cout << "Export count is not 1. Report this. " << image.export_count << std::endl;
		return false;
	}*/
	...
	while (relocation_count > 0)
	{
		if (size > 0)
		{
			...
			if (offset != 0)
			{
				...
				{
					std::cout << "IAT relocation (TODO)" << std::endl;
					*data_pointer = data;
				}
				else if (data >= code_start && data < code_end)
				{
					std::cout << "Code relocation (TODO)" << std::endl;
					*data_pointer = data;
				}
				else
				{
...
void loader::patch_imports(E32Image& image)
{
	...
	for (s32 i = 0; i < image.dll_count; i++)
	{
		...
		if (module_id == 0xFF)
		{
			std::cout << "Module Found: " << name.c_str() << std::endl;
		}
